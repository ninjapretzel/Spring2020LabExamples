...
Shader "FX/StarNest Projection" {
	Properties {
		...
		_Direction ("Fixed Direction", Vector) = (0, -1, 0, 0)
		_DirectionBlend ("Camera Direction Blend", Range(0,1)) = 0
		
		_UpperRange ("Upper Range", Float) = 15
		
		//2D: Zoom level
		_Zoom ("Point Zoom", Float) = -1
		_DirZoom ("Direction Zoom", Float) = 1
		_PointOffset ("Point offset along direction", Range(-5,5)) = 0
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			float3 dir = float3(IN.viewDir);
			dir *= (dot(IN.worldNormal, IN.viewDir) < 0.0) ? -1.0 : 1.0;
			dir = lerp(_Direction.xyz, dir, _DirectionBlend) * _DirZoom;
			...
			float3 from = _Center.xyz + zoom *
			#ifdef WORLDSPACE
				IN.worldPos;
			#else 
				mul(unity_WorldToObject, float4(IN.worldPos,1)).xyz;
			#endif
